  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.emptyText}>Brak notatek. Dodaj pierwszą!</Text>}
        renderItem={({ item }) => (
          <TouchableOpacity 
            style={styles.card} 
...
function AddNoteScreen({ navigation }) {
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.btnSecondary} onPress={getLocation} disabled={isLocating}>
        {isLocating ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnText}>Pobierz GPS</Text>}
      </TouchableOpacity>
    ...
    <NotesContext.Provider value={{ notes, setNotes }}>
      <NavigationContainer>
        <Stack.Navigator screenOptions={{ headerStyle: { backgroundColor: '#0066cc' }, headerTintColor: '#fff' }}>
          <Stack.Screen name="Home" component={HomeScreen} options={{ title: 'Moje Notatki' }} />
          <Stack.Screen name="Details" component={DetailsScreen} options={{ title: 'Szczegóły' }} />
          <Stack.Screen name="AddNote" component={AddNoteScreen} options={{ title: 'Nowa Notatka' }} />
        </Stack.Navigator>
      </NavigationContainer>
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: '#fff', padding: 16, borderRadius: 8, marginBottom: 12, minHeight: 60, elevation: 2 },
  cardTitle: { fontSize: 18, fontWeight: 'bold', color: '#333' },
  cardSub: { fontSize: 14, color: '#666', marginTop: 4 },
  emptyText: { textAlign: 'center', marginTop: 20, fontSize: 16, color: '#888' },
  fab: { position: 'absolute', right: 20, bottom: 20, backgroundColor: '#0066cc', width: 60, height: 60, borderRadius: 30, justifyContent: 'center', alignItems: 'center', elevation: 5 },
  ...
  input: { backgroundColor: '#fff', padding: 12, borderRadius: 8, fontSize: 16, marginBottom: 12, borderWidth: 1, borderColor: '#ddd', minHeight: 48 },
  textArea: { height: 120, textAlignVertical: 'top' },
  locText: { fontSize: 16, color: '#444', marginBottom: 12, fontStyle: 'italic' },
  btnPrimary: { backgroundColor: '#0066cc', padding: 16, borderRadius: 8, alignItems: 'center', minHeight: 48, marginTop: 12 },
  btnSecondary: { backgroundColor: '#28a745', padding: 16, borderRadius: 8, alignItems: 'center', minHeight: 48 },
